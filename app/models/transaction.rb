  ...
  def self.transfer(from_account, to_account, amount, type, writing_id = nil)
    ...
    if @trans_type.a2a?
      ...
      @transaction = Transaction.new(from_account: from_account, to_account: to_account, transaction_type_id: @trans_type.id,
                         ...
                         transaction_description: "Pending: Payment from #{@from_acc.user.email} to teacher #{@to_acc.user.email}")
      @transaction.save
      @from_acc.balance -= amount
      @from_acc.save
      ...
      @transaction.transaction_description = "Settle: Payment from #{@from_acc.user.email} to teacher #{@to_acc.user.email}"
      @transaction.save
      @to_acc.balance += amount
      @to_acc.save
    end
